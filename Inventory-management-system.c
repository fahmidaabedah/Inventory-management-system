...
void addItem() {
    ...
    if (fp) {
        ...
        printf("Creating new inventory file...\n");
    }
...
void editItem() {
    ...
    rewind(fp);  // Move pointer to beginning again to modify the selected item
...
void deleteItem() {
    ...
    // Load all items that belong to the current user
    while (fread(&item, sizeof(struct Item), 1, fp)) {
        if (strcmp(item.username, loggedInUsername) == 0) {
            items[count++] = item;
        }
    ...
    // Sort items by ID (ascending order)
    for (int i = 0; i < count - 1; i++) {
        for (int j = i + 1; j < count; j++) {
            if (items[i].id > items[j].id) {
                struct Item temp = items[i];
    ...
    // Write all items (excluding the one to be deleted) to a new file
    fp = fopen(INVENTORY_FILE, "rb");
    ...
    while (fread(&item, sizeof(struct Item), 1, fp)) {
        ...
        fwrite(&item, sizeof(struct Item), 1, tempFp);
    }
    ...
    if (found) {
        ...
        printf("Item not found or you don't have permission to delete it.\n");
    }
}
